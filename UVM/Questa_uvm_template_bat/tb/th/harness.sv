...
module harness;
    ...
    initial begin
        clk = 0;
        forever #5 clk = ~clk; // 时钟周期 10ns [cite: 4, 5, 6]
    end
    ...
    initial begin
        ...
        uvm_config_db#(virtual slaver_interface)::set(null,"uvm_test_top.env.slv_agt.mon", "vif", output_if);
        uvm_config_db#(virtual bus_if)::set(null,"uvm_test_top.env.bus_agt.drv","vif",b_if);
    end

    initial begin
        run_test("base_test"); // UVM 工厂现在能通过 tc_pkg 找到此类 [cite: 9, 15]
    end
